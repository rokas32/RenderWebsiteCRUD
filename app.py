...
def validate_product_input(data):
    ...
    if stock_quantity is not None:
        try:
            # Check if it's a float that isn't a whole number (e.g., 10.5)
            ...
            # Coerce to integer if it's a valid number representation
    ...
    return "Valid", None # No error found
...
def create_product():
    ...
    # Ensure the mandatory 'name' field is present
    if 'name' not in data:
    ...
    # Create the Product instance, using defaults if fields are missing
    new_product = Product(
        name=data['name'],
...
def list_products():
    ...
    # Use the to_dict method to format the output for JSON
...
def update_product(product_id):
    ...
    # Validate the incoming data before applying updates
    validation_message, error_code = validate_product_input(data)
    if error_code:
        return jsonify({"error": validation_message}), error_code
    
    # Update fields only if they are present in the PUT request payload
    if 'name' in data:
